...
var todotxtData = $(this).siblings('textarea').val();
var todotxtListData = `<div class="row-parent">
                        <div class="list-row">
                        <div class="list-num"></div>
                        <div class="list-data">`+ todotxtData+`</div>
                        <div class="edit-todo">&#9998;</div>
                        </div>
                      <div class="list-error"></div></div>`;
if($.trim(todotxtListData)=='')
{
    $(this).parents('.todo-content').find('.error').text('You must enter something!');
}
else{
     $(this).parents('.todo-content').find('.todo-list').append(todotxtListData );
    i++
    $(this).parents('.todo-content').find('.error').empty();
}
